...
-(void)layoutSubviews
{
    ...
    if([self.gridViewDataSource respondsToSelector:@selector(numberOfGridRows)])
        gridRows = (int)[self.gridViewDataSource numberOfGridRows];
    
    if([self.gridViewDataSource respondsToSelector:@selector(numberOfGridColumns)])
        gridColumns = (int)[self.gridViewDataSource numberOfGridColumns];
   ...
   // NSLog(@"current grid: %@",currentGridIndexPath);
    ...
    int minGridRow = (int)[currentGridIndexPath indexAtPosition:0]-1;
    int maxGridRow = (int)[currentGridIndexPath indexAtPosition:0]+1;
    int minGridColumn = (int)[currentGridIndexPath indexAtPosition:1]-1;
    int maxGridColumn = (int)[currentGridIndexPath indexAtPosition:1]+1;
    ...
    for(int r=MAX(0,minGridRow);r<MIN(gridRows,maxGridRow+1);r++) {
        ...
        for(int c=MAX(0,minGridColumn);c<MIN(gridColumns,maxGridColumn+1);c++) {
            ...
            {
                subGridRows = (int)[self.gridViewDataSource numberOfRowsForGridAtIndexPath:gridIndexPath];
            }
            ...
            {
                subGridCols = (int)[self.gridViewDataSource numberOfColumnsForGridAtIndexPath:gridIndexPath];
            }
